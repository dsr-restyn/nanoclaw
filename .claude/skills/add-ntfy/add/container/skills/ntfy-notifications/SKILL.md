...
```bash
# Simple notification
curl -d "Meeting in 15 minutes" ntfy.sh/$NTFY_TOPIC

# With title and priority
curl -H "Title: Reminder" -H "Priority: high" -d "Meeting in 15 minutes" ntfy.sh/$NTFY_TOPIC

# With tags/emoji
curl -H "Tags: white_check_mark" -d "Report finished" ntfy.sh/$NTFY_TOPIC

# Low priority (no sound)
curl -H "Priority: low" -d "Background task complete" ntfy.sh/$NTFY_TOPIC
```
