...
/** Scan static/ for custom Creations (subdirs with creation.json). */
...
<body>
  ...
  <div class="section">
    <h2>${i + 2}. ${c.name.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</h2>
    <img src="/pair/creation/${c.slug}/qr?token=${token}" alt="${c.name.replace(/"/g, '&quot;')} QR" width="300" height="300">
    <div class="note">${c.description.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</div>
  </div>
  `).join('')}
  ${VOICE_ENABLED ? `
  <div class="section">
